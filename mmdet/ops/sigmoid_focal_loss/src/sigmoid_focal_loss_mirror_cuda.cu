...
__global__ void SigmoidFocalLossForwardMirror(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    // Indicator function: positive(same class), negative(different class)
    // Don't ask me why targets start from 1.
    scalar_t c1 = (t == (d + 1));
    ...
    scalar_t tp = c1 * (p >= thresh) + c2 * (p <= (1 - thresh));
    scalar_t tn = c1 * (p < thresh) + c2 * (p > (1 - thresh));
    ...
    // change gamma value
    scalar_t term3 = powf((1. - p), gamma2) * logf(max(p, FLT_MIN));
        
    losses[i] = 0.0;
    losses[i] += -c1 * term1 * tp * zp;
    losses[i] += -c1 * term1 * tn * zp;
    losses[i] += -c2 * term2 * tp * zn;
    losses[i] += -c2 * term3 * tn * zn;


  }  // CUDA_1D_KERNEL_LOOP
}  // SigmoidFocalLossForwardMirror

template <typename scalar_t>
__global__ void SigmoidFocalLossBackwardMirror(const int nthreads, 
                                                const scalar_t *logits, 
                                                const long *targets, 
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    // 
    scalar_t term3 =
        powf((1. - p), gamma2) * (1. - p - (p * gamma2 * logf(max(p, FLT_MIN))));

    d_logits[i] = 0.0;
    d_logits[i] += -c1 * term1 * tp * zp;
    d_logits[i] += -c1 * term1 * tn * zp;
    d_logits[i] += -c2 * term2 * tp * zn;
    d_logits[i] += -c2 * term3 * tn * zn;
    d_logits[i] = d_logits[i] * d_losses[i];

  }  // CUDA_1D_KERNEL_LOOP
}  // SigmoidFocalLossBackwardMirror
...
at::Tensor SigmoidFocalLossMirror_forward_cuda(const at::Tensor &logits,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      logits.type(), "SigmoidFocalLossMirror_forward", [&] {
        SigmoidFocalLossForwardMirror<scalar_t><<<grid, block>>>(
...
at::Tensor SigmoidFocalLossMirror_backward_cuda(const at::Tensor &logits,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      logits.type(), "SigmoidFocalLossMirror_backward", [&] {
        SigmoidFocalLossBackwardMirror<scalar_t><<<grid, block>>>(
